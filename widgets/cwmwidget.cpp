...
void CwmWidget::md5sumVerify()
{
    ...
    if (result.contains("FAILED") || result.contains("WARNING" || result.contains("No such file")))
    {
...
void CwmWidget::backupAvailable()
{
    ...
    else if (which == "backupVerify")
    {
        ...
        phone->setPath(this->threadSdcard->backupPath);
        QList<File> *fileList = NULL;
        fileList = phone->getFileList();
        while (!fileList->isEmpty())
        {
            File img = fileList->takeFirst();
            if (img.fileSize == "0")
            {
                if (QMessageBox::critical(this, tr("Backup ROM:"),tr("The image \"") + img.fileName + tr("\" is 0 size file!\nThis Backup is invalid!!!\n\nDo you want to recreate it now?"),QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes)
                    this->on_buttonBackup_pressed();
                else
                {
...
void CwmWidget::lineBackupClear()
{
    this->ui->lineBackup->clear();
